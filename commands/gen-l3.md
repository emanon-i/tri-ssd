...
0. **Phase 1 は必ず「自己検証環境セットアップ」とする**
   - L2の技術スタック・アーキテクチャから、エージェントがコマンド実行で自律的に確認できる検証手段をすべて洗い出す
   - コーディングエージェントが Phase 2 以降で人間に確認を求めず自己検証できる基盤を構築する
   - このフェーズは全プロジェクト共通で必須

1. **残りのフェーズ数は？**（自己検証フェーズを含めて合計 3〜5 フェーズ）
   - 小規模（機能5個以下）: 自己検証 + 1〜2フェーズ
   - 中規模（機能6〜15個）: 自己検証 + 2〜3フェーズ
   - 大規模（機能16個以上）: 自己検証 + 3〜4フェーズ

2. **各フェーズの境界は？**
   - 「この時点でユーザーに見せられるか？」で判断
   - 技術的な区切りより、ビジネス価値の区切りを優先

3. **Phase 2（実質的な最初の機能フェーズ）に何を入れるか？**
   - 早く成果を出せる機能を優先
   - 依存関係の根幹となる機能を含める
</thinking_process>

### Step 1.5: 自己検証環境セットアップフェーズの生成（必須）

**必ず最初のフェーズ（PH-xxx-001）として生成する。**

L2 の技術スタック・アーキテクチャを参照し、コーディングエージェントが以降のフェーズで人間の判断を仰がず自律的に自己検証できる環境を整備するフェーズを生成する。

#### 検証手段の決定方法

L2 の技術スタック・アーキテクチャを読み、そのプロジェクトで**コーディングエージェントがコマンド実行で自律的に確認できる検証手段**をすべて洗い出す。

判断の観点（L2の内容に該当するものをすべて含める）:

| 観点 | 例 |
|------|-----|
| テスト実行 | ユニットテスト、結合テスト、E2Eテスト等、L2の技術に適したテストランナー |
| 静的解析 | リンター、フォーマッター、型チェッカー等 |
| ビルド | コンパイル、バンドル、トランスパイル等 |
| UI/画面確認 | スナップショットテスト、Storybook、ビジュアルリグレッション等（Webフロントエンドの場合） |
| アプリ起動確認 | dev server起動、デスクトップアプリのパッケージング、モバイルのエミュレータビルド等 |
| CI/CD | L2でCI/CDが定義されている場合のパイプライン設定 |

**重要**: 上記は観点の例であり、固定リストではない。L2に記載された技術構成から、エージェントが自分で実行して結果を判定できる検証手段を網羅的に選定すること。

#### 受け入れ条件の書き方

各検証手段について「コマンドを実行し、成功すること」を受け入れ条件とする。

```markdown
**受け入れ条件**:
- [ ] [検証ツールA] が実行でき、エラーなしで完了する
- [ ] [検証ツールB] が実行でき、エラーなしで完了する
- [ ] ...（L2の技術構成に応じて必要な分だけ）
```
...
```markdown
## Exit Criteria
- [ ] L2で採用した技術に対応する全検証ツールが正常に動作する
- [ ] 各検証コマンドの実行方法がREADMEまたはスクリプトとして記録されている
```
...
フェーズごとに独立ファイルを生成（**Phase 1 は必ず自己検証環境セットアップ**）:
```
docs/l3_phases/
├── PH-20250203-001_self-verification-setup.md   # 必須: 自己検証環境
├── PH-20250203-002_mvp.md
├── PH-20250203-003_beta.md
└── PH-20250203-004_release.md
```
...
例: `PH-20250203-001_self-verification-setup.md`（Phase 1 は必ず自己検証環境）
