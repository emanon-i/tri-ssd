...
0. **Phase 1 は必ず「自己検証環境セットアップ」とする**
   - L2の技術スタック・アーキテクチャから、テスト・リンター・型チェッカー・ビルド等の検証手段を特定
...
#### L2技術スタックからの検証手段マッピング

| L2の技術スタック | セットアップ対象 |
|-----------------|-----------------|
| Node.js / TypeScript | jest or vitest, eslint, tsc, package.json scripts |
| Python | pytest, ruff or flake8, mypy, pyproject.toml |
| Go | go test, golangci-lint, go vet |
| Rust | cargo test, clippy, cargo build |
| その他 | L2記載の技術に応じた検証ツール |

#### このフェーズに含める機能

1. **テスト環境構築**: テストランナーの設定、サンプルテストの作成・実行確認
2. **リンター・フォーマッター設定**: L2の技術に対応したリンター設定・実行確認
3. **型チェッカー設定**（静的型付け言語の場合）: 設定ファイル作成・実行確認
4. **ビルドパイプライン確認**: ビルドコマンドが正常に通ることを確認
5. **CI設定**（L2でCI/CDが定義されている場合）: GitHub Actions等の設定

#### 受け入れ条件の例

```markdown
**受け入れ条件**:
- [ ] テストランナーが実行でき、サンプルテストがパスする
- [ ] リンターが実行でき、エラーなしで完了する
- [ ] 型チェッカーが実行でき、エラーなしで完了する（該当する場合）
- [ ] ビルドが成功する
```
...
```markdown
## Exit Criteria
- [ ] 全検証ツール（テスト・リンター・型チェッカー・ビルド）が正常に動作する
- [ ] `git diff` で変更内容を確認し、不要なファイルが含まれていない
```
