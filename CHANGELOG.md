...
- **gen-l3: 最初のフェーズに自己検証環境セットアップを必須化**
  - Phase 1 として必ず自己検証環境（テスト・リンター・型チェッカー・ビルド）のセットアップフェーズを生成
  - L2の技術スタックから検証手段を自動マッピング（Node.js→jest/eslint/tsc、Python→pytest/ruff/mypy 等）
  - コーディングエージェントが Phase 2 以降で人間の判断を仰がず自律的に自己検証できる基盤を構築
